...
const Points = () => {
    ...
    useEffect(() => {
        async function loadPosition() {
            const { status } = await Location.requestPermissionsAsync();//Pedindo a permissão para obter a localização 

            if (status !== 'granted') {
                Alert.alert('Oooops...', 'Precisamos de sua localização para realizar algumas funções.')
                return;
            }

            const location = await Location.getCurrentPositionAsync();//Pegando a localização atual
        ...
        loadPosition();//Permite usar a async function dentro do useEffect
    }, [])
        ...
        <>
            <View style={styles.container}>
                <TouchableOpacity onPress={handleNavigateBack}>
                    <Icon name='arrow-left' size={20} color="#3acb79"/>
                </TouchableOpacity>
                ...
                <View style={styles.mapContainer}>
                    {/* MapView é da lib react-native-maps */}
                    { initialPosition[0] !== 0 && (
            ...
            <View style={styles.itemsContainer}>
                <ScrollView 
                    ...
                    contentContainerStyle={{ paddingHorizontal:32}}//Interpreta como parte do conteúdo
                >
                    {items.map(item => (
